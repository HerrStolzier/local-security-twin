...
- Codex App Server und Codex SDK bleiben vorerst Entwicklungswerkzeuge, kein Bestandteil von Sento Guard. Ein eingebauter Agent-Server würde für den MVP zu viel Cloud-, Auth-, Rechte- und Vertrauensfläche öffnen; sinnvoller ist höchstens später ein interner Guardrail-Review-Workflow außerhalb der App.
- Externe Cybersecurity-Skill-Bibliotheken sind für Sento Guard zuerst Entwicklungs- und Review-Material, keine Produktabhängigkeit. Nützlich sind Struktur, defensive Review-Fragen, Framework-Mappings und Threat-Intel-Qualitätsregeln; ungefilterte Skills, Scripts oder offensive Playbooks gehören nicht in die App.
...
- Defensive Reviews brauchen eine feste Vorlage, damit aus Angreiferdenken immer ein sicherer Output entsteht: read-only Sensoridee, Nutzer-Checkliste, sicherere Guided Action, dokumentierte Grenze oder Entscheidung, die Funktion noch nicht zu bauen.
...
- Threat Context muss klein und quellenbewusst starten: Quelle, Abrufzeitpunkt, Frischegrenze, Confidence, lokaler Bezug und sichtbare Grenze sind wichtiger als ein großer Threat-Intel-Stack.
- AI-/MCP- und Developer-Tool-Risiken gehören erst dann als Produktbefund in Sento Guard, wenn lokale Betroffenheit belegbar ist. Deklarierte Tool-Fähigkeiten, Tool-Namen oder Social-Media-CVE-Warnungen sind keine Sicherheitsbeweise; spätere Sensoren müssen Konfiguration, Transportart, lauschende Oberfläche, Dependency-Versionen und effektiv sichtbare Berechtigungen trennen.
...
- Geführte Hygiene-Fragen brauchen direkt in der Karte einen kurzen Grund, warum Sento fragt. Sonst wirken Ja/Nein-Antworten wie eine interne Checkliste statt wie eine ehrliche lokale Einordnung mit sichtbarer Grenze.
- Gespeicherte Hygiene-Antworten brauchen immer einen sichtbaren Änderungs- und Zurücknehmen-Weg. Eine lokale Nutzerangabe ist kein dauerhafter Befund, sondern eine bewusst änderbare Selbstauskunft.
